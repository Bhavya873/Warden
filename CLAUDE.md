...
**Key invariant:** the Ribbon filter must never produce a false negative — that's what `tests/test_ribbon_filter.py::test_no_false_negatives` and `tests/test_no_permanent_gridlock`/`test_no_collisions_*` exist to guard. Filter *staleness* (a robot's local filter being out of date) is a different, accepted risk: it can't actually cause a collision in this simulation, because the live occupancy check inside `GridWorld.tick()` is the final authority on every move, independent of whatever any policy approved — a stale filter produces a "near-miss" (a rejected attempt), never a real collision.

Measured (not estimated) traffic-reduction and CPU figures are in `README.md`'s "Measured results" section, sourced from `results/benchmark_results.csv` / `results/coordinator_load.png` (`python -m sim.scenarios` regenerates both) — read that section before quoting a number from this project anywhere.
